...
Sortie :
    ml/model/model_disponibilite.pkl  ← modèle + métadonnées
    ml/model/rapport_evaluation.txt  ← rapport complet
"""
...
# Ligne FEATURES dans ml/train_model.py
...
def evaluate(model, X_train, X_test, y_train, y_test) -> dict:
    ...
    log.info("Rapport de classification :\n%s",
             classification_report(y_test, y_pred,
                                   target_names=["Disponible", "Occupée"]))
...
def feature_importance(model) -> list:
    ...
    for feat, imp in pairs:
        bar = "█" * int(imp * 40)
        log.info("  %-16s : %5.1f%%  %s", feat, imp * 100, bar)
    return pairs
...
def save_report(metrics: dict, importances: list, n_total: int):
    ...
    for feat, imp in importances:
        bar = "█" * int(imp * 40)
        lines.append(f"  {feat:<16} : {imp*100:5.1f}%  {bar}")

    lines += [
        "",
        "INTERPRÉTATION",
        "-" * 40,
        "  L'heure est la variable dominante (≈74%) — les bornes suivent",
